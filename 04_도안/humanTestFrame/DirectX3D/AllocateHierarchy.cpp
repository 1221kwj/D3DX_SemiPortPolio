...
{
	//받아와서 실제로 할당되는 것이 아니기 때문에 직접 할당해야됨
	Bone* frame = new Bone;
	//초기화 개념?
...
STDMETHODIMP AllocateHierarchy::CreateMeshContainer ( LPCSTR Name
													...
													, LPD3DXMESHCONTAINER * ppNewMeshContainer )
{
	...
	for ( DWORD i = 0; i < NumMaterials; ++i )
	{
		...
		materialTexture->SetMaterial ( ( D3DMATERIAL9 ) pMaterials[i].MatD3D );

		std::string sFile ( pMaterials[i].pTextureFilename );
		//folder <- skinnedMesh에서 set으로 넣어줬음
		std::string sFullPath = folder + std::string ( "\\" ) + sFile;
		//texture
		materialTexture->SetTexture ( g_pTextureManager->GetTexture ( sFullPath ) );
		pBoneMesh->materialTextureList.push_back ( materialTexture );
		*/
	}
	...
	pBoneMesh->orgMesh->CloneMeshFVF(pBoneMesh->orgMesh->GetOptions()
	...
	*/
	...
	//pBoneMesh->ppBoneMatrixPtrs = new D3DXMATRIX*[dwNumBone];
	//pBoneMesh->pBoneOffsetMatrices = new D3DXMATRIX[dwNumBone];
	//pBoneMesh->pCurrentBoneMatrices = new D3DXMATRIX[dwNumBone];
