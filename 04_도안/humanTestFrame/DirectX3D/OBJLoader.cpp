#include "stdafx.h"
#include "OBJLoader.h"

#include "MatrialTexture.h"
#include "Group.h"

OBJLoader::OBJLoader ()
{
}


OBJLoader::~OBJLoader ()
{
}

void OBJLoader::SetPath ( char * path )
{
}

void OBJLoader::Load ( IN char * filename, IN D3DXMATRIXA16 * pmat, OUT std::vector<Group*>& groupList )
{
	std::vector<D3DXVECTOR3> VList;
	std::vector<D3DXVECTOR2> VTList;
	std::vector<D3DXVECTOR3> VNList;
	std::vector<VertexPNT> PNTList;
	std::vector<DWORD> attrList;

	std::string materialName;

	FILE* fp = NULL;

	if( path.empty() ) 
		fopen_s ( &fp, filename, "r" );
	else {
		std::string fullPath;
		fullPath = path + filename;
		fopen_s ( &fp, fullPath.c_str(), "r" );
	}
	
	

	while ( true )
	{
		if ( feof ( fp ) )
			break;

		char szTemp[1024];

		fgets ( szTemp, 1024, fp );

		if ( szTemp[0] == '#' )
		{
			continue;
		}
		else if ( szTemp[0] == 'm' )
		{
			char szMtlPath[1024];
			sscanf ( szTemp, "%*s %s", szMtlPath );
			LoadMtlLib ( szMtlPath );


		}
		else if ( szTemp[0] == 'g' )
		{
			if ( PNTList.empty () )
				continue;
			Group* pGroup = new Group();
			pGroup->SetPNTVerts ( PNTList );
			pGroup->SetVB ();
			pGroup->SetMaterialTexture ( mapMaterialTexture[materialName] );
			groupList.push_back ( pGroup );
			PNTList.clear ();
		}
		else if ( szTemp[0] == 'v' )
		{
			if ( szTemp[1] == 't' )
			{
				float u, v;
				sscanf ( szTemp, "%*s %f %f %*f", &u, &v );
				VTList.push_back ( D3DXVECTOR2 ( u, v ) );
			}
			else if ( szTemp[1] == 'n' )
			{
				float x, y, z;
				sscanf ( szTemp, "%*s %f %f %f", &x, &y, &z );
				VNList.push_back ( D3DXVECTOR3 ( x, y, z ) );
			}
			else // v
			{
				float x, y, z;
				sscanf ( szTemp, "%*s %f %f %f", &x, &y, &z );
				VList.push_back ( D3DXVECTOR3 ( x, y, z ) );
			}
		}
		else if ( szTemp[0] == 'u' )
		{
			char scanedString[1024];
			sscanf ( szTemp, "%*s %s", scanedString );
			materialName = scanedString;
		}
		else if ( szTemp[0] == 'f' )
		{
			int aIndex[3][3];
			sscanf ( szTemp, "%*s %d/%d/%d %d/%d/%d %d/%d/%d",
				&aIndex[0][0], &aIndex[0][1], &aIndex[0][2],
				&aIndex[1][0], &aIndex[1][1], &aIndex[1][2],
				&aIndex[2][0], &aIndex[2][1], &aIndex[2][2] );
			

			for ( int i = 0; i < 3; ++i )
			{
				VertexPNT v;
				v.p = VList[aIndex[i][0] - 1];
				v.t = VTList[aIndex[i][1] - 1];
				v.n = VNList[aIndex[i][2] - 1];

				if ( pmat )
				{
					D3DXVec3TransformCoord ( &v.p, &v.p, pmat );
					D3DXVec3TransformNormal ( &v.n, &v.n, pmat );
				}
				PNTList.push_back ( v );
			}
		}
	}
	

	mapMaterialTexture.clear ();

	fclose ( fp );

}

LPD3DXMESH OBJLoader::LoadMesh(IN char * filename, IN D3DXMATRIXA16 * pmat, OUT std::vector<MaterialTexture*>& mtList)
{
	std::vector<D3DXVECTOR3> VList;
	std::vector<D3DXVECTOR2> VTList;
	std::vector<D3DXVECTOR3> VNList;
	std::vector<VertexPNT> PNTList;
	std::vector<DWORD> attrList;

	std::string materialName;

	FILE* fp = NULL;

	if (path.empty()) fopen_s(&fp, filename, "r");
	else 
	{
		std::string fullPath;
		fullPath = path + filename;
		fopen_s(&fp, fullPath.c_str(), "r");
	}

	while (true)
	{
		if (feof(fp))
			break;

		char szTemp[1024];

		fgets(szTemp, 1024, fp);

		if (szTemp[0] == '#')
		{
			continue;
		}
		else if (szTemp[0] == 'm')
		{
			char szMtlPath[1024];
			sscanf(szTemp, "%*s %s", szMtlPath);
			LoadMtlLib(szMtlPath);
			//??????????????????
			mtList.resize(mapMaterialTexture.size());
			for each(auto mt in mapMaterialTexture)
			{
				mtList[mt.second->GetMTRef()] = mt.second;
			}

		}
		else if (szTemp[0] == 'g')
		{
			if (PNTList.empty()) continue;
			//Group* pGroup = new Group();
			//pGroup->SetPNTVerts(PNTList);
			//pGroup->SetVB();
			//pGroup->SetMaterialTexture(mapMaterialTexture[materialName]);
			//groupList.push_back(pGroup);
			//PNTList.clear();
		}
		else if (szTemp[0] == 'v')
		{
			if (szTemp[1] == 't')
			{
				float u, v;
				sscanf(szTemp, "%*s %f %f %*f", &u, &v);
				VTList.push_back(D3DXVECTOR2(u, v));
			}
			else if (szTemp[1] == 'n')
			{
				float x, y, z;
				sscanf(szTemp, "%*s %f %f %f", &x, &y, &z);
				VNList.push_back(D3DXVECTOR3(x, y, z));
			}
			else // v
			{
				float x, y, z;
				sscanf(szTemp, "%*s %f %f %f", &x, &y, &z);
				VList.push_back(D3DXVECTOR3(x, y, z));
			}
		}
		else if (szTemp[0] == 'u')
		{
			char scanedString[1024];
			sscanf(szTemp, "%*s %s", scanedString);
			materialName = scanedString;
		}
		else if (szTemp[0] == 'f')
		{
			int aIndex[3][3];
			sscanf(szTemp, "%*s %d/%d/%d %d/%d/%d %d/%d/%d",
				&aIndex[0][0], &aIndex[0][1], &aIndex[0][2],
				&aIndex[1][0], &aIndex[1][1], &aIndex[1][2],
				&aIndex[2][0], &aIndex[2][1], &aIndex[2][2]);
			//attributeList 
			attrList.push_back(mapMaterialTexture[materialName]->GetMTRef());

			for (int i = 0; i < 3; ++i)
			{
				VertexPNT v;
				v.p = VList[aIndex[i][0] - 1];
				v.t = VTList[aIndex[i][1] - 1];
				v.n = VNList[aIndex[i][2] - 1];

				if (pmat)
				{
					D3DXVec3TransformCoord(&v.p, &v.p, pmat);
					D3DXVec3TransformNormal(&v.n, &v.n, pmat);
				}
				PNTList.push_back(v);
			}
		}
	}
	//Mesh
	LPD3DXMESH mesh = nullptr;

	//Mesh create
	HRESULT hr = D3DXCreateMeshFVF(PNTList.size() / 3, PNTList.size(), D3DXMESH_MANAGED, VertexPNT::FVF, D3DDevice, &mesh);

	//VertextBuffer
	VertexPNT* vertBuffer = nullptr;
	mesh->LockVertexBuffer(0, (LPVOID*)&vertBuffer);
	//memcpy - 메모리 영역을 복사하는 함수
	memcpy(vertBuffer, &PNTList[0], PNTList.size() * sizeof(VertexPNT));
	mesh->UnlockVertexBuffer();

	//IndexBuffer
	WORD* indexBuffer = nullptr;
	mesh->LockIndexBuffer(0, (LPVOID*)&indexBuffer);
	for (int i = 0; i < PNTList.size(); i++)
	{
		indexBuffer[i] = i;
	}
	mesh->UnlockIndexBuffer();

	//AttributeBuffer
	DWORD* attrBuffer = nullptr;
	mesh->LockAttributeBuffer(0, &attrBuffer);
	memcpy(attrBuffer, &attrList[0], attrList.size() * sizeof(DWORD));
	mesh->UnlockAttributeBuffer();

	//최적화해주기
	//폴리곤은 삼각형이기 때문에 그옆에 붙어있는 삼각형이 3개 이기 때문에 곱하기 3
	std::vector<DWORD>adjBuffer(mesh->GetNumFaces() * 3);
	//                    Epsilon
	mesh->GenerateAdjacency(0.0f, &adjBuffer[0]);

	mesh->OptimizeInplace(D3DXMESHOPT_ATTRSORT |	//속성으로 삼각형을 정렬, 속성 테이블 생성
		D3DXMESHOPT_COMPACT |						//메쉬에서 이용되지 않는 인덱스와 버텍스를 제거
		D3DXMESHOPT_VERTEXCACHE,					//버텍스 캐쉬의 히트율을 높인다?(메모리 사용의 단계를 올려준다)
		&adjBuffer[0],
		0,
		0,
		0);
	//위에 최적화 때문에 mesh를 이용한다.!
	
	mapMaterialTexture.clear();
	
	fclose(fp);
	
	return mesh;
}

void OBJLoader::LoadMtlLib ( char * filename )
{
	FILE* fp = NULL;

	//레퍼런스 카운트
	int i = 0;

	if ( path.empty () )
		fopen_s ( &fp, filename, "r" );
	else {
		std::string fullPath;
		fullPath = path + filename;
		fopen_s ( &fp, fullPath.c_str (), "r" );
	}


	std::string sMtlName;

	while ( true )
	{
		if ( feof ( fp ) )
			break;

		char szTemp[1024];
		
		fgets ( szTemp, 1024, fp );

		if ( szTemp[0] == '#' )
		{
			continue;
		}
		else if ( szTemp[0] == 'n' )
		{
			char szMtlName[1024];
			sscanf ( szTemp, "%*s %s", szMtlName );
			sMtlName = szMtlName;

			SAFE_RELEASE ( mapMaterialTexture[sMtlName] );
			/////////////////////////////////////////////////////
			mapMaterialTexture[sMtlName] = new MaterialTexture();
			//카운트 넣어주고                                       
			mapMaterialTexture[sMtlName]->SetMTRef(i);            
			//카운트 증가                                          
			i++;                                                 
			/////////////////////////////////////////////////////
		}
		else if ( szTemp[0] == 'K' )
		{
			if ( szTemp[1] == 'a' )
			{
				float r, g, b;
				sscanf ( szTemp, "%*s %f %f %f", &r, &g, &b );
				D3DMATERIAL9& stMtl = mapMaterialTexture[sMtlName]->GetMaterial ();
				stMtl.Ambient.r = r;
				stMtl.Ambient.g = g;
				stMtl.Ambient.b = b;
				stMtl.Ambient.a = 1.0f;
			}
			else if ( szTemp[1] == 'd' )
			{
				float r, g, b;
				sscanf ( szTemp, "%*s %f %f %f", &r, &g, &b );
				D3DMATERIAL9& stMtl = mapMaterialTexture[sMtlName]->GetMaterial ();
				stMtl.Diffuse.r = r;
				stMtl.Diffuse.g = g;
				stMtl.Diffuse.b = b;
				stMtl.Diffuse.a = 1.0f;
			}
			else if ( szTemp[1] == 's' )
			{
				float r, g, b;
				sscanf ( szTemp, "%*s %f %f %f", &r, &g, &b );
				D3DMATERIAL9& stMtl = mapMaterialTexture[sMtlName]->GetMaterial ();
				stMtl.Specular.r = r;
				stMtl.Specular.g = g;
				stMtl.Specular.b = b;
				stMtl.Specular.a = 1.0f;
			}
		}
		else if ( szTemp[0] == 'm' )
		{
			char szTexturePath[1024];
			sscanf ( szTemp, "%*s %s", szTexturePath );

			mapMaterialTexture[sMtlName]->SetTexture ( g_pTextureManager->GetTexture ( szTexturePath ) );
		}
	}

	fclose ( fp );

}
