...
void OBJLoader::Load(IN char* szFolder, IN char* szFile, IN D3DXMATRIXA16 * pmat, OUT std::vector<Group*>& groupList)
{
	...
	//{
	//	std::string fullPath;
	//	fullPath = path + szFullPath.c_str();
	...
	while (true)
	{
		...
		else if (szTemp[0] == 'm')
		{
			...
			sscanf_s(szTemp, "%*s %s", szMtlPath, sizeof( szMtlPath ));
		...
		else if (szTemp[0] == 'g')
		{
			...
			pGroup->SetMapVertexList( VList );
			pGroup->SetTileCount( ( int )sqrt( VList.size() ) );
		...
		else if (szTemp[0] == 'u')
		{
			...
			sscanf_s(szTemp, "%*s %s", scanedString, sizeof( scanedString ));
...
void OBJLoader::LoadMtlLib(char * filename)
{
	...
	while (true)
	{
		...
		{
			char szMtlName[1024];
			sscanf_s( szTemp, "%*s %s", szMtlName, sizeof( szMtlName ) );
			sMtlName = szMtlName;
		...
		else if (szTemp[0] == 'm')
		{
			...
			sscanf_s(szTemp, "%*s %s", szTexturePath, sizeof( szTexturePath ));

			mapMaterialTexture[sMtlName]->SetTexture(TEXTUREMANAGER->GetTexture(szTexturePath));
		}
	}
